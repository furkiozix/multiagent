...
class ClassifierML:
    ...
    def evaluation_for_weights(self, model, strategy):
        ...
        if strategy == 1:
          ...
          # Accuracy
            result = model.score(datasetc.X_val, Y_val)
        elif strategy == 4:
...
class CompContNet1(FipaContractNetProtocol):
    ...
    def handle_all_proposes(self, proposes):
        ...
        for message in proposes:
            ...
            Plt.plot(fpr2, tpr2, label='%s (F1 = %0.2f)' % (labels[i-1], f1), linestyle='-.', color='red')
        ...
        if other_proposers != []:
            ...
            f1 = f1_score(datasetc.Y_test, yhat, average='weighted')  # 'weighted','macro', 'micro' gibi farklı ortalamalar da kullanılabilir.
            
            display_message(self.agent.aid.name, 'Confusion matrix is : {pot} '.format(pot=cm))
            display_message(self.agent.aid.name, 'Classification error rate is : {pot} '.format(pot=loss))
            display_message(self.agent.aid.name, 'F1 Score is : {pot} '.format(pot=f1))  # F1 Skorunu göster
